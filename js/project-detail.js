(function () {
  ...
  const sectionLabels = {
    ja: { overview: "概要", problem: "課題", solution: "解決方法", technical: "技術詳細", result: "結果 / 効果", learned: "学んだこと", back: "← 選考作品へ戻る", media: "PROJECT MEDIA · TODO", official: "公式トレーラーを見る ↗", store: "STOVEで無料プレイ ↗", nda: "公開情報と本人の担当領域のみを掲載しています。社内アセット、コード、ドキュメントは含まれません。" },
    en: { overview: "Overview", problem: "Problem", solution: "Solution", technical: "Technical Breakdown", result: "Result / Impact", learned: "What I Learned", back: "← Back to Selected Work", media: "PROJECT MEDIA · TODO", official: "Watch Official Trailer ↗", store: "Play free on STOVE ↗", nda: "This page contains public information and a description of my responsibilities only. No internal assets, code or documents are included." },
    ko: { overview: "개요", problem: "문제", solution: "해결 방법", technical: "기술 상세", result: "결과 / 영향", learned: "배운 점", back: "← 주요 작업으로 돌아가기", media: "PROJECT MEDIA · TODO", official: "공식 트레일러 보기 ↗", store: "스토브에서 무료 플레이 ↗", nda: "공개 정보와 본인의 담당 영역만 포함합니다. 회사 내부 에셋, 코드와 문서는 포함하지 않습니다." },
    zh: { overview: "概述", problem: "问题", solution: "解决方案", technical: "技术解析", result: "结果 / 影响", learned: "经验总结", back: "← 返回精选作品", media: "PROJECT MEDIA · TODO", official: "观看官方预告片 ↗", store: "在STOVE免费游玩 ↗", nda: "本页仅包含公开信息与本人职责说明，不包含公司内部资产、代码或文档。" }
  };
  ...
  function render() {
    ...
    <div class="project-content shell">${sections.map((key, index) => {
      ...
      const external = key === "overview" ? `<p class="external-source">${project.externalUrl ? `<a class="button button-ghost" href="${escapeHtml(project.externalUrl)}" target="_blank" rel="noreferrer">${escapeHtml(labels.official)}</a>` : ""}${project.storeUrl ? `<a class="button button-ghost" href="${escapeHtml(project.storeUrl)}" target="_blank" rel="noreferrer">${escapeHtml(labels.store)}</a>` : ""}</p>` : "";
